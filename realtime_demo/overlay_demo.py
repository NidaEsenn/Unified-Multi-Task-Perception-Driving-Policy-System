...
from perception.lane_unet import build_model as build_lane_model, predict as predict_lane
from perception.drivable_area_unet import build_model as build_drivable_model, predict as predict_drivable
...
def main() -> None:
    ...
    parser.add_argument("--weights-det", type=str, default="yolov8n.pt", help="Detector weights (TODO)")
    parser.add_argument("--weights-lane", type=str, default="", help="Lane model weights (TODO)")
    parser.add_argument("--weights-drivable", type=str, default="", help="Drivable model weights (TODO)")
    args = parser.parse_args()

    src: str | int = int(args.source) if str(args.source).isdigit() else args.source
    cap = cv2.VideoCapture(src)
    detector = YOLOv8Detector(args.weights_det)
    lane_model = build_lane_model()
    drivable_model = build_drivable_model()
